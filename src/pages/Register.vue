<template>
  <div id="register" class="pt-24 pb-32 px-4">
    <div class="box mx-auto bg-white rounded-lg pt-12 pb-16 px-10">
      <h3 class="mb-6">Zarejestruj się</h3>
      <CzarnaInput
        class="mb-6"
        placeholder="E-mail"
      ...
      />
      <label class="flex items-center text-sm sm:text-base mb-6 agree-terms">
        <input type="checkbox" class="checkbox rounded-sm border-red-500 checked:bg-red-500 checked:hover:bg-red-500 checked:focus:bg-red-500 checked:focus:ring-red-500 focus:ring-red-500 mr-2" v-model="agreeTerms" />
        Akceptuje warunki&nbsp;<a class="czarna-red" href="">regulaminu</a>
      </label>
      <button
        ...
        @click="onRegister()"
        :disabled="isInvalid || !agreeTerms"
      >
        Zarejestruj
      </button>
      <span class="login-user">Masz już konto? Zaloguj się&nbsp;<a @click="$router.push('/login')">tutaj</a></span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    onRegister: function() {
      ...
      ApiService
        .postRequest(GLOBAL.host_url + API_CONSTANT.REGISTER, payload)
        .then(() => {
          this.$router.push('/');
        })
        .catch(err => {
...
<style scoped>
  ...
  div#register div.box h3 {
    font-size: 1.5rem;
    font-weight: 620;
    line-height: 1.75rem;
  }

  div#register label.agree-terms {
    font-size: 12px;
    line-height: 15px;
    color: #14161F;
  }

  div#register div.box span.login-user {
    font-size: 14px;
    line-height: 36px;
    color: #525F7A;
  }

  div#register div.box a {
